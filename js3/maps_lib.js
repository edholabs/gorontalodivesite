...
var MapsLib = {
  ...
  map_centroid:       new google.maps.LatLng(0.639285, 122.0319069), //center that your map defaults to
  locationScope:      "gorontalo",      //geographical area appended to all address searches
  ...
  },
  {
    ...
    if ($.address.parameter('Harga Sedang') == "1")
      $("#cbType2").attr("checked", true);
    else
      $("#cbType2").attr("checked", false);

    if ($.address.parameter('Harga Mewah') == "1")
      $("#cbType3").attr("checked", true);
    else
      $("#cbType3").attr("checked", false);


     if ($.address.parameter('Eropa') == "1")
      $("#cbType5").attr("checked", true);
    else
      $("#cbType5").attr("checked", false);

    if ($.address.parameter('Italia') == "1")
      $("#cbType6").attr("checked", true);
    else
      $("#cbType6").attr("checked", false);


    if ($.address.parameter('Reservasi') == "1")
      $("#cbType8").attr("checked", true);
    else
      $("#cbType8").attr("checked", false);

    if ($.address.parameter('Musik') == "1")
      $("#cbType9").attr("checked", true);
    else
      $("#cbType9").attr("checked", false);

     if ($.address.parameter('Makan Siang') == "1")
      $("#cbType11").attr("checked", true);
    else
      $("#cbType11").attr("checked", false);

    if ($.address.parameter('Makan Malam') == "1")
      $("#cbType12").attr("checked", true);
    else
      $("#cbType12").attr("checked", false);
  ...
  //-----custom filters for point data layer
    ...
    var type_column = "'Harga'";  // -- note use of single & double quotes for two-word column header
    var tempWhereClause = [];
    if ( $("#cbType1").is(':checked')) tempWhereClause.push("Harga Murah");
    if ( $("#cbType2").is(':checked')) tempWhereClause.push("Harga Sedang");
    if ( $("#cbType3").is(':checked')) tempWhereClause.push("Harga Mewah");
    whereClause += " AND " + type_column + " IN ('" + tempWhereClause.join("','") + "')";

    var type_column = "'Menu'"; 
    var tempWhereClause = [];
    if ( $("#cbType4").is(':checked')) tempWhereClause.push("Siap Saji");
    if ( $("#cbType5").is(':checked')) tempWhereClause.push("Eropa");
    if ( $("#cbType6").is(':checked')) tempWhereClause.push("Italia");
    whereClause += " AND " + type_column + " IN ('" + tempWhereClause.join("','") + "')";

    var type_column = "'Fasilitas'"; 
    var tempWhereClause = [];
    if ( $("#cbType7").is(':checked')) tempWhereClause.push("Wifi");
    if ( $("#cbType8").is(':checked')) tempWhereClause.push("Reservasi");
    if ( $("#cbType9").is(':checked')) tempWhereClause.push("Musik");
    whereClause += " AND " + type_column + " IN ('" + tempWhereClause.join("','") + "')";

    var type_column = "'Jam_layanan'"; 
    var tempWhereClause = [];
    if ( $("#cbType10").is(':checked')) tempWhereClause.push("Makan Pagi");
    if ( $("#cbType11").is(':checked')) tempWhereClause.push("Makan Siang");
    if ( $("#cbType12").is(':checked')) tempWhereClause.push("Makan Malam");
    whereClause += " AND " + type_column + " IN ('" + tempWhereClause.join("','") + "')";
  ...
  displaySearchCount: function(json) {
    ...
    $( "#result_box" ).fadeOut(function() {
        $( "#result_count" ).html(MapsLib.addCommas(numRows) + " Restoran" + " ditemukan");
      });
    $( "#result_box" ).fadeIn();
  },
  ...
  getList: function(whereClause) {
    ...
    var selectColumns = "Nama_Restoran, Harga, Fasilitas, Cluster, Website";
  ...
  displayList: function(json) {
    ...
    if (rows == null) {
      ...
      // rows[row][0] = Nama_Restoran
      // rows[row][1] = Alamat
      // rows[row][2] = Kota
      // rows[row][3] = Kabupaten
      // rows[row][4] = Website
      // rows[row][5] = Telepon
      // rows[row][6] = Latitude
      // rows[row][7] = Longitude
      // rows[row][8] = Pemandangan
      // rows[row][9] = Website
      // rows[row][10] = Cluster
